<template>
  <div>
    ...
    <el-dialog :lock-scroll='true' class='abow_dialog' top='7vh' title="焦炭质量预测结果(人工智能算法)" custom-class="userDig" center :show-close='true' :closeOnPressEscape='true' :close-on-click-modal="true" :visible.sync="editCoalDetailVisible" :append-to-body="true" width="75%" height='40%'>
        ...
        <p></p>
        <el-row>
          <span style="font-weight:normal">本数据采用Lasso回归算法进行预测，总平均预测误差百分比  ——ㅤCRI: {{AverageCRIError}}%， CSR: {{AverageCSRError}}%</span>
        </el-row>
    </el-dialog>
    </div>
        <div> <!--切换按钮-->
...
export default {
  ...
  methods: {
    ...
    sendPredictedData() { // 发送待预测的数据
      const result = this.$http.post('getCokeQualityfyeResultAIXGBoost', this.multipleSelectionData)
      result.then(res => {
        if (res.data[0] === 'Error') {

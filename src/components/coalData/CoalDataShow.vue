<template>
<div>
  <!-- 使用pl-table的属性 :row-height为行高 用row-style实现隔行变色，返回值为字符串 -->
  <pl-table :row-style="tableRowClassName" :data-changes-scroll-top='false' ref='CoalTable' big-data-checkbox highlight-current-row :row-height="33" :data = 'coalList.slice((cur_page-1)*pageSize,cur_page*pageSize)' use-virtual border :header-cell-style="{background:'white',color:'#494949',textAlign:'center', border:'1px solid #BFBFBF',fontSize: '14px'}" :height="height" :key='Math.random()' :default-sort="{prop: 'id', order: 'ascending'}" size='mini' :cell-style="{padding:'0px',fontSize: '14px'}" fixedColumnsRoll>
                <pl-table-column align="center" fixed label='序号' prop='id' key="1" ></pl-table-column>
                ...
                <pl-table-column align="center" fixed label='煤种存储量' prop='store_number' key="4"></pl-table-column> <!-- 加prop可以直接渲染值 -->
                ...
                <template slot-scope="scope">
                    <el-button @click="showDetailedDialog(scope.row.id)" type="text" size="small">查看</el-button>
                    <el-button type="text" size="small">编辑</el-button>
                    <el-button type="text" size="small" @click='removeCoalById(scope.row.id)'>删除</el-button>
                </template>
                </pl-table-column>
            </pl-table>
                  <!-- 分页栏 -->
  ...
  methods: {
    ...
    async getCoalList() {
      await this.$http.get('coalData', { params: this.queryInfo_get }).then(ret => {
        ...
        console.log(this.editCoalDetailVisible)
      }
      )
    },
    // 根据ID删除对应的用户信息
